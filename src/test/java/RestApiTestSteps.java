...
public class RestApiTestSteps {
    ...
    private static final MPost EXPECTED_RECEIVED_POST = JSONConverter.serializeJSONString(MPost.class, testData.getValue("/ExpectedData/ExpectedNumberedPostExisting").toString());
    private static final MUser EXPECTED_USER = JSONConverter.serializeJSONString(MUser.class, testData.getValue("/ExpectedData/ExpectedUser").toString());
    private static final MPost POSTING_POST = new MPost(
            1,
    ...
    public static void allPostsTest(){
        ...
        ValidatableResponse vResponse = Request.get(Endpoints.POSTS)
    ...
    public static void existingPostTest(){
        ...
        Request.get(Endpoints.POST_BY_ID, EXISTING_POST_ID)
    ...
    public static void nonExistingPostTest(){
        ...
        Request.get(Endpoints.POST_BY_ID, NON_EXISTING_POST_ID)
    ...
    public static void allUsersAndIndividualUserTest(){
        ...
        ValidatableResponse vResponse = Request.get(Endpoints.USERS)
                .statusCode(HttpStatus.SC_OK)
                .contentType(ContentType.JSON);
        MUser[] users = vResponse.extract().as(MUser[].class);
        MUser userFoundedFromAllUsers = SearchHelper.findUserById(users, EXISTING_USER_ID);
        ...
        vResponse = Request.get(Endpoints.USER_BY_ID, EXISTING_USER_ID)
                .statusCode(HttpStatus.SC_OK);
        MUser userFoundedDirectly = vResponse.extract().as(MUser.class);
